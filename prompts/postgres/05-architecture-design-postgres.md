## ASP.NET Core / EF Core / PostgreSQL / DDD / optionale Multi-Tenancy / Audit / Caching
...
6. Welche sollten besser Services, Contexts, Provider, Decorators oder Infrastructure Components sein?
...
14. Wie wird Reference Data bereitgestellt und gecacht?
...
Insbesondere soll zwischen folgenden Begriffen unterschieden werden:
...
Ein **Aggregate Repository** repräsentiert Persistence für Aggregate Roots.

Ein **Reference Data Store** stellt persistierte, überwiegend lesend verwendete Reference Data bereit.

Ein **Reference Data Provider** stellt Reference Data für die Application-Schicht bereit und kann dabei Cache und Store koordinieren.

Ein **Reference Data Cache** ist ausschließlich eine Performance-/Caching-Komponente und keine Persistence-Abstraktion.
...
│ Reference Data Store Implementation           │
│ Cache                                         │
...
# 17. Enum Entity vs. Reference Data
...
Reference Data bezeichnet hierbei relativ stabile, kontrollierte Daten, die andere fachliche Daten klassifizieren, beschreiben oder validieren.
...
Eine Reference Data Entity kann fachlich gegebenenfalls administrativ gepflegt werden.
...
# 20. Repository- und Datenzugriffsarchitektur
...
> Welche Abstraktionen sind tatsächlich Repositorys und welche sollten besser Services, Contexts, Provider, Decorators oder Infrastructure Components sein?
...
Reference Data Access
        │
        ▼
IReferenceDataStore<T,TKey>

Reference Data Provision
        │
        ▼
IReferenceDataProvider<T,TKey>
...
> `IReferenceDataStore` ist kein Aggregate Repository.

> `IReferenceDataProvider` ist kein Repository.

> `IReferenceDataCache` ist keine Persistence-Abstraktion.
...
# 25. Reference Data Store

Entwickle ein separates Konzept für **Reference Data**.

Reference Data umfasst relativ stabile, kontrollierte Daten, die andere fachliche Daten klassifizieren, beschreiben oder validieren.
...
Das `ReferenceDataStore` ist bewusst **kein generisches Aggregate Repository**.

Es soll nur die für Reference Data erforderlichen Leseoperationen abstrahieren.

Bewerte, ob ein konkreter Store pro Reference-Data-Typ erforderlich ist oder ob ein generischer Store ausreicht.

---

# 26. Reference Data Provider

Verwende für die Application-seitige Bereitstellung von Reference Data den Begriff:

```text
ReferenceDataProvider
```

Bewerte insbesondere folgende Verantwortung:
...
- Reference Data bereitstellen
- Cache verwenden können
- Cache Misses behandeln
- den Persistence-Zugriff kapseln
- die Application nicht von Cache-Technologie abhängig machen
- keine Aggregate-Persistence ersetzen

Beurteile, ob der Provider:
...
```text
Infrastructure-facing Application Abstraction
```

sein sollte.

Treffe eine konkrete Entscheidung.
...
Der Cache darf nicht als Repository bezeichnet werden, wenn er keine Persistence-Verantwortung besitzt.

---

# 28. Reference Data Provider Flow
...
Begründe, ob der Cache vor dem Store liegen sollte oder als Decorator um den Store umgesetzt werden sollte.
...
```text
Cached Store Decorator
```

und entscheide dich für eine Variante.

Die finale Architektur darf nicht gleichzeitig mehrere konkurrierende Abstraktionen für dieselbe Verantwortung enthalten.
...
# 35. Lazy Loading vs. Startup Preloading
...
# 36. Mehrere Application Instances
...
# 40. PostgreSQL-spezifische Persistence
...
# 46. Repository- und Datenzugriffsmatrix

Erstelle und korrigiere folgende Matrix:

| Typ | Datenzugriff | CRUD | Cache | Tenant |
|---|---|---:|---:|---:|
| Aggregate Root | Aggregate Repository | fachlich begrenzt | optional | optional |
| Child Entity | keines | Nein | Nein | über Aggregate |
| Enum Entity | Reference Data Store | Nein | Ja | normalerweise Nein |
| Reference Data | Reference Data Store | fachlich begrenzt | optional | optional |
...
Die finale Architektur darf nicht zusätzlich ein `ICacheRepository` einführen, wenn `IReferenceDataCache` bereits die benötigte Cache-Abstraktion darstellt.
...
2. Reference Data über `ReferenceDataProvider` bezieht
...
## 51.4 Reference Data / Enum Diagram
...
Unterscheide im Diagramm ausdrücklich zwischen:
...
wenn beide dargestellt werden.

---

## 51.5 Reference Data Access Diagram
...
## 51.7 Multi-Instance Cache Diagram
...
## 51.9 Single-Tenant vs. Multi-Tenant
...
# 52. Abstraktions-Level in PlantUML
...
23. Reference Data Store
24. Reference Data Provider
25. Reference Data Cache
26. Cache Decorator
27. Cache-aside
28. Read-through
29. Lazy Loading
...
| Reference Data Store | optional | optional |
| Reference Data Provider | optional | optional |
...
# 55. Finale Architekturentscheidung
...
- Was wird unter Reference Data verstanden?
- Wie unterscheidet es sich von Enum Entities?
- Wie unterscheidet es sich von normalen Lookup-/CRUD-Daten?
- Welche Daten werden über `ReferenceDataStore` gelesen?
- Welche Verantwortung hat `ReferenceDataProvider`?
- Welche Verantwortung hat `ReferenceDataCache`?
- Welche Komponenten dürfen Reference Data verändern?

## Repository / Data Access

- Was ist das Aggregate Repository?
- Was ist der Reference Data Store?
- Gibt es ein Tenant Repository?
- Gibt es ein Cache Repository?
- Wenn nein, warum nicht?
- Welche Komponenten sind Repositorys und welche bewusst keine Repositorys?
...
Reference Data, Enum Entity, Lookup Entity, Seed Data.

## 10. Repository / Data Access Architecture

Aggregate Repository, Reference Data Store, Tenant Repository.

## 11. Reference Data Provider

Provider, Cache und Store sowie deren Verantwortungsgrenzen.
...
## 14. Specification / CQRS

Klare Empfehlung.

## 15. PostgreSQL / Npgsql

Provider-spezifische Entscheidungen.

## 16. EF Core Configuration
...
## 22. Entscheidungs-Matrix

Alternativen und finale Entscheidung.

## 23. Finale Gesamtarchitektur
...
21. Reference Data Access darf nicht mit Aggregate Persistence vermischt werden.
22. Ein Cache darf nicht als Repository bezeichnet werden, wenn er keine Persistence-Verantwortung besitzt.
23. `ReferenceDataProvider`, `ReferenceDataStore` und `ReferenceDataCache` müssen klar getrennte Verantwortlichkeiten besitzen.
24. Es darf nicht mehrere konkurrierende Abstraktionen für dieselbe Reference-Data-Verantwortung geben.

---

# 61. Wichtigste finale Anforderung
...
Wie wird Reference Data über den Provider bereitgestellt?
...
Reference Data        = fachliches Datenkonzept
        │
        ▼
ReferenceDataProvider = stellt die Daten bereit
        │
        ├──────────────► ReferenceDataCache
        │
        └──────────────► ReferenceDataStore
